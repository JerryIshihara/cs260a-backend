...
def parse_comment(comment):
    return {'comment_key': comment.comment_key, 'aws_key': comment.aws_key, 'user_id': comment.user_id, 'comment': comment.comment, 'created_at': comment.created_at, 'avatar_url': comment.avatar_url}
...
class VideoComments(db.Model):
    ...
    @staticmethod
    def insert_video_comment(aws_key, user_id, comment, avatar_url):
        cur_time = get_current_time()
        new_comment = VideoComments(aws_key = aws_key, user_id = user_id, comment = comment, created_at = cur_time, avatar_url = avatar_url)
